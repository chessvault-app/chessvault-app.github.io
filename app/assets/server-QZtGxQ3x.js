...
`).flatMap(e=>{try{let t=JSON.parse(e);return typeof t?.id==`string`?[t]:[]}catch{return[]}})},s=(e,t)=>typeof e.seen==`number`?t.slice(e.seen).some(t=>t.id===e.id):t.some(t=>t.id===e.id&&typeof t.at==`string`&&t.at>=e.at),c=(e,t,n,r)=>{let i=e.offered?.[t];return!i||i.id===r||s(i,n)?null:i.id},l=(e,t,n,r)=>{let i=Object.entries(e.offered??{}).filter(([e,n])=>e!==t&&!s(n,r));i.push([t,{id:n,at:new Date().toISOString(),seen:r.length}]),a({...e,offered:Object.fromEntries(i.slice(-16))})},u=(e,t)=>e.prepare(`SELECT id, fen, moves, rating, popularity, plays, themes, game_url, opening_tags FROM puzzles WHERE id = ?`).get(t),d=(e,t=o())=>{let n=t.filter(e=>e.counted!==!1);if(n.length===0)return null;let r=[...new Set(n.map(e=>e.id))],i=new Map;for(let t=0;t<r.length;t+=500){let n=r.slice(t,t+500),a=e.prepare(`SELECT id, themes FROM puzzles WHERE id IN (${n.map(()=>`?`).join(`,`)})`).all(...n);for(let e of a)i.set(e.id,e.themes.split(` `).filter(Boolean))}let a=new Map,s=0;for(let e of n){e.win&&(s+=1);for(let t of i.get(e.id)??[]){let n=a.get(t)??{attempts:0,wins:0};n.attempts+=1,e.win&&(n.wins+=1),a.set(t,n)}}let c=s/n.length,l=null;for(let[e,t]of a){if(t.attempts<Qr)continue;let n=t.wins/t.attempts;n>=c||(!l||n<l.wins/l.attempts)&&(l={theme:e,...t})}return l},f=(e=o())=>{let t=new Map,n=new Set;for(let r of e)t.set(r.id,r.win),r.counted!==!1&&n.add(r.id);return[...t].filter(([e,t])=>!t&&n.has(e)).map(([e])=>e)},p=(e=o())=>{let t=new Map,n=new Set;for(let r of e){let e=t.get(r.id)??[],i=r.at;e.push({win:r.win,...typeof i==`string`?{at:i}:{}}),t.set(r.id,e),r.counted!==!1&&n.add(r.id)}let r=[];for(let[e,i]of t){if(!n.has(e))continue;let t=fe(i);t!==null&&r.push({id:e,due:t})}return r.sort((e,t)=>e.due<t.due?-1:+(e.due>t.due))},m=e=>{if(typeof e.skill==`number`)return{...e,skill:e.skill,skillAttempts:e.skillAttempts??0};let t=ii,n=0;for(let e of o()){if(e.counted===!1)continue;let r=e.puzzleRating;typeof r==`number`&&(t=oi(t,n,r,e.win),n+=1)}let r={...e,skill:t,skillAttempts:n};return a(r),r},h=null,g=()=>h||(I(e)?(h=new Dn(e,{readonly:!0,fileMustExist:!0}),h):null),_=()=>{h?.close(),h=null},v=new G,y=null,b=()=>{let t={startedAt:Date.now(),running:!0,progress:{phase:`downloading`,bytes:0,total:0},error:null};y=t;let n=K(X,`server`,`build-puzzles.mjs`),r=I(n)?[n,`--progress-json`]:[`--import`,`tsx`,`scripts/build-puzzles.ts`,`--progress-json`],i=(0,sn.spawn)(process.execPath,r,{cwd:X,stdio:[`ignore`,`pipe`,`pipe`]}),a=``;i.stdout.on(`data`,e=>{a+=e.toString();let n=a.split(`
...
`.replace(/SELECT pos, uci, eb,[\s\S]*?GROUP BY pos, uci, eb;/,`SELECT p.pos AS pos, p.uci AS uci,
    ...
    FROM plies p JOIN games g ON g.id = p.game_id
    GROUP BY p.pos, p.uci;`);function Pa(e){if(!e.prepare(`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'plies'`).get())return{indexed:!1,plies:0,stale:!1};let t=e.prepare(`SELECT value FROM meta WHERE key = 'plies'`).get(),n=Number(e.prepare(`SELECT value FROM meta WHERE key = 'indexed_through'`).get()?.value),r=n&&Number(e.prepare(`SELECT MAX(id) AS n FROM games`).get().n)||0;return{indexed:!0,plies:Number(t?.value)||e.prepare(`SELECT COUNT(*) AS n FROM plies`).get().n,stale:n>0&&r>n}}var Fa=class extends Error{constructor(){super(`the request was abandoned`),this.name=`AbortError`}},Ia=e=>e instanceof Error&&e.name===`AbortError`,La=new Map;function Ra(e){let t=La.get(e);return t||(t=new Dn(e,{readonly:!0,fileMustExist:!0}),La.set(e,t)),t}async function za(e,t,n,r,i){if(i?.aborted)throw new Fa;await Promise.resolve();let a=Ra(e).prepare(t);return r===`get`?a.get(...n):a.all(...n)}function Ba(e){return{all:(t,n,r)=>za(e,t,n,`all`,r),get:(t,n,r)=>za(e,t,n,`get`,r)}}function Va(e){for(let[t,n]of La)(e===void 0||t===e)&&(n.close(),La.delete(t));return Promise.resolve()}var Ha=`
...
`))n.trim()&&(e.log.push(n),e.lineAt=Date.now());e.log.length>100&&e.log.splice(0,e.log.length-100)};f.stdout.on(`data`,p),f.stderr.on(`data`,p),f.on(`error`,t=>{e.log.push(`could not start the job: ${t.message}`),d(-1)}),f.on(`close`,d)},r=(e,n,r)=>{let i={kind:`build`,name:e,startedAt:Date.now(),running:!0,exitCode:null,log:[],lineAt:Date.now(),stopped:!1,child:null};r&&u(e),t(i,`build-refgames.mjs`,`scripts/build-refgames.ts`,[...n,`--name`,e,...r?[`--append`]:[]],[`build`,...n,`--name`,e,...r?[`--append`]:[],`--data`,$t],async t=>{await u(e);let n=`${o(e)}.building`;if(!r&&t===0&&I(n))try{Wt(n,o(e))}catch{i.log.push(`could not swap in the new database: rebuild after a restart`)}if(!r&&i.stopped)for(let e of[n,`${n}-wal`,`${n}-shm`,`${n}-journal`])L(e,{force:!0})})};_.post(`/refgames/build`,async t=>{if(Q?.running)return t.json({error:`a build is already running`},409);let i=await t.req.json().catch(()=>null),a=i?.sources??(()=>{try{return A(Zt).filter(e=>e.toLowerCase().endsWith(`.pgn`))}catch{return[]}})();if(a.length===0)return t.json({error:`no PGN files to index`},400);let s=[];for(let n of a){let r=e(n);if(!r)return t.json({error:`invalid or missing source: ${n}`},400);s.push(r)}let c=a.length===1?a[0].replace(/\.pgn$/i,``):`refgames`,l=i?.name??(eo.test(c)?c:`refgames`);if(!eo.test(l))return t.json({error:`invalid database name`},400);let u=i?.mode===`append`;return u&&!I(o(l))?t.json({error:`no such database to add to`},400):(M(n,{recursive:!0}),r(l,s,u),t.json({started:!0,name:l,mode:u?`append`:`replace`}))}),_.post(`/refgames/optimize`,async e=>{if(Q?.running)return e.json({error:`a build is already running`},409);let n=(await e.req.json().catch(()=>null))?.db??s()[0];if(!n||!eo.test(n)||!s().includes(n))return e.json({error:`no such database`},400);let r={kind:`optimize`,name:n,startedAt:Date.now(),running:!0,exitCode:null,log:[],lineAt:Date.now(),stopped:!1,child:null};return u(n),t(r,`optimize-refgames.mjs`,`scripts/optimize-refgames.ts`,[n],[`optimize`,n,`--data`,$t],()=>void u(n)),e.json({started:!0,name:n})}),_.post(`/refgames/index-positions`,async e=>{if(Q?.running)return e.json({error:`a build is already running`},409);let n=(await e.req.json().catch(()=>null))?.db??s()[0];return!n||!eo.test(n)||!s().includes(n)?e.json({error:`no such database`},400):(t({kind:`index`,name:n,startedAt:Date.now(),running:!0,exitCode:null,log:[],lineAt:Date.now(),stopped:!1,child:null},`index-refgames-positions.mjs`,`scripts/index-refgames-positions.ts`,[n],[`index`,n,`--data`,$t],()=>void u(n)),e.json({started:!0,name:n}))});let i=/positions: (\d+)% ([^—…]+?)\s*(?:—|…)/,a=e=>{for(let t=e.length-1;t>=0;--t){let n=i.exec(e[t]);if(n)return{label:n[2],percent:Number(n[1])}}return null},c=/([\d,]+) of ([\d,]+) games/,l=e=>{for(let t=e.length-1;t>=0;--t){let n=c.exec(e[t]);if(n)return{done:Number(n[1].replaceAll(`,`,``)),total:Number(n[2].replaceAll(`,`,``))}}return null};_.post(`/refgames/build/stop`,e=>!Q?.running||!Q.child?e.json({error:`no build is running`},409):(Q.stopped=!0,Q.log.push(`stopped from the app`),Q.lineAt=Date.now(),Q.child.kill(),e.json({stopped:!0,name:Q.name}))),_.get(`/refgames/build/status`,e=>e.json(Q?{running:Q.running,kind:Q.kind,name:Q.name,exitCode:Q.exitCode,stopped:Q.stopped,seconds:(Date.now()-Q.startedAt)/1e3,progress:l(Q.log),phase:a(Q.log),quietSeconds:(Date.now()-Q.lineAt)/1e3,log:Q.log.slice(-15)}:{running:!1})),_.delete(`/refgames/:name`,async e=>{let t=e.req.param(`name`);return eo.test(t)?Q?.running&&Q.name===t?e.json({error:`that database is being built right now`},409):I(o(t))?(await u(t),await Ma(o(t)),L(o(t)),e.json({deleted:t})):e.json({error:`no such database`},404):e.json({error:`invalid database name`},400)})}_.post(`/refgames/fast-scan`,async e=>{let n=await e.req.json().catch(()=>null);if(!n||typeof n.on!=`boolean`)return e.json({error:`expected db & on`},400);let r=t?``:n.db??``;if(!t&&(!eo.test(r)||!s().includes(r)))return e.json({error:`no such database`},404);let i=o(r),a=c(r);if(!a)return e.json({error:`no such database`},503);let l=m(a);if(n.on&&l.scan_pack!==`1`)return e.json({error:`this database has no scan index yet, re-run Index positions`},409);let u=new Dn(i);try{u.pragma(`busy_timeout = 30000`),n.on?u.prepare(`INSERT OR REPLACE INTO meta (key, value) VALUES ('fast_scan', '1')`).run():u.prepare(`DELETE FROM meta WHERE key = 'fast_scan'`).run()}finally{u.close()}if(n.on)try{let{games:t,bytes:n}=await ka(i);return e.json({on:!0,resident:{games:t,bytes:n}})}catch(t){return console.error(`refgames: could not load the index for ${r}: ${t.message}`),e.json({error:`could not load the index`},500)}return await Ma(i),e.json({on:!1})}),_.get(`/refgames`,async e=>{if(t){let t=g(e);if(!t)return e.json({ready:!1});let n=m(t.db);return e.json({ready:!0,games:Number(n.games??0),sources:n.sources??``})}let r=s().flatMap(e=>{let t=c(e);if(!t)return[];let n=m(t),r=Pa(t);return[{name:e,games:h(e,t),sources:n.sources??``,bytes:N(o(e)).size,builtAt:n.built_at??null,indexed:r.indexed,stale:r.stale,positions:r.plies,packed:n[ua]===`1`,fastScan:n.fast_scan===`1`,resident:ja(o(e))!==null}]}),i=n===Ua?vi():null;return e.json({ready:r.length>0,native:i!==null&&await Qa(i)!==null,databases:r})});let v=(e,t)=>{if(!to(e)||uo(e=>e===`band`?void 0:t(e),`g.`).clauses.length>0)return null;let n=oo(t(`band`)),r=e.prepare(`SELECT 1 FROM pragma_table_info('move_counts') WHERE name = 'eb'`).get()!==void 0,i=t=>e.prepare(`SELECT uci, SUM(w) AS w, SUM(d) AS d, SUM(b) AS b FROM move_counts
       ...
       ORDER BY id DESC LIMIT ${Wa}`,[...p,s??2**53-1],i);return e.json({total:_,capped:s===null?v:void 0,nextCursor:y.length===Wa?y.at(-1).id:null,rows:y.map(({moves:e,...t})=>{let n={...t,...b(e)};if(n.opening)return n;let r=x(e);return r?{...n,eco:n.eco??r.eco,opening:r.name}:n})})}));let C=(e,t,n,r)=>{let i=`SELECT p.uci AS uci,
                SUM(g.result = '1-0') AS w,
                SUM(g.result = '1/2-1/2') AS d,
                SUM(g.result = '0-1') AS b
         FROM plies p JOIN games g ON g.id = p.game_id
         WHERE p.pos = ?${t}
         GROUP BY p.uci
         ORDER BY w + d + b DESC, p.uci`;return t=>d(e).all(i,[t,...n],r)};return _.get(`/refgames/explore`,f(async e=>{let t=g(e);if(!t)return e.json({error:`no reference games database`},503);let{db:n}=t;if(lo(e.req.query(`terms`))>co)return e.json({error:`too many search terms`},400);let r=e.req.query(`fen`)?.trim();if(!r)return e.json({error:`expected fen`},400);let i=l(r);if(i.isErr)return e.json({error:`bad fen`},400);let o=p.fromSetup(i.unwrap());if(o.isErr)return e.json({error:`bad position`},400);let s=o.unwrap();if(!Pa(n).indexed)return e.json({indexed:!1,opening:null,games:0,moves:[],topGames:[]});let c=Te(z(s.toSetup())),{clauses:u,binds:f}=uo(t=>e.req.query(t),`g.`,ro(n),Pa(n).indexed),m=u.length?` AND ${u.join(` AND `)}`:``,h=e.req.raw.signal,_=C(t,m,f,h),b=v(n,t=>e.req.query(t)),x=b?b(c):await _(c);x.length===0&&b&&(x=await _(c));let S=x.flatMap(e=>{let t=a(e.uci);return!t||!s.isLegal(t)?[]:[{uci:e.uci,san:T(s,t),w:e.w,d:e.d,b:e.b,total:e.w+e.d+e.b}]}),w=(y(n,t=>e.req.query(t))?.(c)??await d(t).all(`SELECT p.uci AS uci, g.white, g.black, g.white_elo AS whiteElo,
                g.black_elo AS blackElo, g.result, g.date
         FROM plies p JOIN games g ON g.id = p.game_id
         WHERE p.pos = ?${m}
         ORDER BY g.white_elo + g.black_elo DESC, g.id DESC
         LIMIT 8`,[c,...f],h)).filter(e=>{let t=a(e.uci);return t!==void 0&&s.isLegal(t)});return e.json({indexed:!0,opening:Yn(z(s.toSetup()).toString(16)),games:S.reduce((e,t)=>e+t.total,0),moves:S,topGames:w.map(e=>({...e,site:null}))})})),_.post(`/refgames/explore-batch`,f(async e=>{let t=g(e);if(!t)return e.json({error:`no reference games database`},503);let{db:n}=t,r=await e.req.json().catch(()=>null),i=Array.isArray(r?.fens)?r.fens.filter(e=>typeof e==`string`):null;if(!i)return e.json({error:`expected fens`},400);if(i.length>256)return e.json({error:`too many positions`},400);if(lo(e.req.query(`terms`))>co)return e.json({error:`too many search terms`},400);if(!Pa(n).indexed)return e.json({indexed:!1,positions:[]});let{clauses:o,binds:s}=uo(t=>e.req.query(t),`g.`,ro(n),Pa(n).indexed),c=o.length?` AND ${o.join(` AND `)}`:``,u=C(t,c,s,e.req.raw.signal),d=v(n,t=>e.req.query(t)),f=[];for(let e of i){let t=l(e.trim());if(t.isErr){f.push({fen:e,moves:[]});continue}let n=p.fromSetup(t.unwrap());if(n.isErr){f.push({fen:e,moves:[]});continue}let r=n.unwrap(),i=Te(z(r.toSetup())),o=d?d(i):await u(i);o.length===0&&d&&(o=await u(i));let s=o.flatMap(e=>{let t=a(e.uci);return!t||!r.isLegal(t)?[]:[{uci:e.uci,san:T(r,t),w:e.w,d:e.d,b:e.b,total:e.w+e.d+e.b}]});f.push({fen:e,moves:s})}return e.json({indexed:!0,positions:f})})),_.get(`/refgames/deep-search`,async e=>{let t=g(e);if(!t)return e.json({error:`no reference games database`},503);let{db:r}=t,i=e.req.query(`fen`)?.trim(),a=e.req.query(`match`),s=e.req.query(`material`),c=e.req.query(`motif`);if(s!==void 0&&(i!==void 0||a!==void 0))return e.json({error:`material search takes no fen or match`},400);if(c!==void 0&&(i!==void 0||a!==void 0||s!==void 0))return e.json({error:`motif search takes no fen, match or material`},400);if(a!==void 0&&!ue.includes(a))return e.json({error:`bad match mode`},400);let u=a??`exact`,d=s===void 0?null:ae(s);if(s!==void 0&&d===null)return e.json({error:`bad material spec`},400);let f=c===void 0?null:ra(c);if(c!==void 0&&f===null)return e.json({error:`bad motif spec`},400);let m=null,_,v,y;if(f)_=16,v=16,y=f.stable-1;else if(d){let e=ie(d);_=e.hiW,v=e.hiB,y=d.stable-1}else{if(!i)return e.json({error:`expected fen`},400);let t=l(i);if(t.isErr)return e.json({error:`bad fen`},400);let n=p.fromSetup(t.unwrap());if(n.isErr&&u===`exact`)return e.json({error:`bad position`},400);m=n.isErr?ma(t.unwrap(),u):ma(n.unwrap(),u),u===`structure`?(_=16,v=16,y=0):(_=m.w,v=m.b,y=32-m.w-m.b)}let x=de((e.req.query(`q`)??``).trim()),S=x.text?[...x.terms,{kind:`player`,value:x.text}]:x.terms;if(S.length>co)return e.json({error:`too many search terms`},400);let C=t=>t===`terms`?S.length>0?JSON.stringify(S):void 0:e.req.query(t),{clauses:w,binds:T}=uo(C,``,ro(r),Pa(r).indexed),E=w.length?` AND ${w.join(` AND `)}`:``,ee=r.prepare(`SELECT 1 FROM pragma_table_info('games') WHERE name = 'final_wmen'`).get()!==void 0,D=ee?` AND (final_wmen IS NULL OR final_wmen <= ?)
          AND (final_bmen IS NULL OR final_bmen <= ?)
          AND (ply_count IS NULL OR ply_count >= ?)`:``,te=ee?[_,v,y]:[],O=e=>r.prepare(`SELECT value FROM meta WHERE key = ?`).get(e)?.value,ne=o(t.name),re=Number(O(`indexed_through`)??0)>=(r.prepare(`SELECT MAX(id) AS n FROM games`).get().n??0);if(!d&&!f&&u===`exact`&&re&&O(`key_index`)===`1`){let t=r.prepare(`SELECT entries FROM key_index WHERE bucket = ?`).get(ya(m.key32)),n=[];if(t){let e=t.entries,r=e.length/8,{lo:i,hi:a}=xa(m.key32),o=0,s=r;for(;o<s;){let t=o+s>>1;e.readBigUInt64LE(t*8)<i?o=t+1:s=t}let c=-1;for(;o<r;o+=1){let t=e.readBigUInt64LE(o*8);if(t>=a)break;let r=ba(t);r!==c&&(n.push(r),c=r)}}let i=null;if(w.length>0){i=new Set;let e=e=>`SELECT id FROM games WHERE id IN (${Array.from({length:e},()=>`?`).join(`,`)})${E}`,t=n.length>=500?r.prepare(e(500)):null;for(let a=0;a<n.length;a+=500){let o=n.slice(a,a+500),s=(o.length===500?t:r.prepare(e(o.length))).all(...o,...T);for(let e of s)i.add(e.id)}}let a=i?n.filter(e=>i.has(e)):n,o=r.prepare(`SELECT id, white, black, white_elo, black_elo, result, date, event, eco, opening, moves
         FROM games WHERE id = ?`);return e.header(`Content-Type`,`application/x-ndjson`),e.header(`X-Scan-Path`,`key-index`),Qi(e,async t=>{let n=0,r=0;for(let i of a){if(t.aborted||e.req.raw.signal?.aborted)return;r+=1;let a=o.get(i);if(!a)continue;let s=ha(a.moves,m);if(s===null)continue;n+=1;let{moves:c,...l}=a;if(await t.writeln(JSON.stringify({type:`game`,ply:s,...l,...b(c)})),n>=200)break}await t.writeln(JSON.stringify({type:`done`,scanned:r,total:a.length,matched:n,exhaustive:n<200}))})}let oe=!1;if(!f&&re&&O(`fast_scan`)===`1`&&O(`scan_pack`)===`1`)try{await ka(ne),oe=ja(ne)!==null}catch(e){console.error(`deep-search (${t.name}): resident index unavailable: ${e.message}`)}if(oe){let n=null;if(w.length>0){let e=`${O(`indexed_through`)??``}|${O(`indexed_at`)??``}`,t=`${ne}|${E}|${JSON.stringify(T)}`,i=Za.get(t);i&&i.stamp===e?n=i.ids:(n=new Float64Array(r.prepare(`SELECT id FROM games WHERE 1${E} ORDER BY id`).all(...T).map(e=>e.id)),Za.set(t,{ids:n,stamp:e}),Za.size>8&&Za.delete(Za.keys().next().value))}let i=n?n.length:h(t.name,r),a=r.prepare(`SELECT id, white, black, white_elo, black_elo, result, date, event, eco, opening, moves
         FROM games WHERE id = ?`);return e.header(`Content-Type`,`application/x-ndjson`),e.header(`X-Scan-Path`,`resident`),Qi(e,async t=>{let r=[],o=0,s=!1,c=!1,l=Aa(ne,d?{spec:d}:{target:m},n,{onHits:e=>{for(let t=0;t<e.length;t+=2)r.push({id:e[t],ply:e[t+1]})},onBad:e=>{for(let t of e)r.push({id:t,ply:null})},onProgress:e=>{o=e}});l.done.then(e=>{o=e.scanned,s=!0}).catch(()=>{c=!0,s=!0});let u=0,f=-1;for(;;){if(t.aborted||e.req.raw.signal?.aborted){l.cancel();return}for(;r.length>0&&u<200;){let e=r.shift(),n=a.get(e.id);if(!n)continue;let i=e.ply;if(e.ply===null&&(i=d?_a(n.moves,d):ha(n.moves,m)),i===null)continue;u+=1;let{moves:o,...s}=n;await t.writeln(JSON.stringify({type:`game`,ply:i,...s,...b(o)}))}if(u>=200&&(l.cancel(),r.length=0),s&&r.length===0)break;o!==f&&(f=o,await t.writeln(JSON.stringify({type:`progress`,scanned:o,total:i,matched:u}))),await new Promise(e=>setTimeout(e,15))}c||await t.writeln(JSON.stringify({type:`done`,scanned:o,total:i,matched:u,exhaustive:u<200}))})}let ce=n===Ua?vi():null,le=ce?await Qa(ce):null,fe=ce&&le&&le.deep===qa&&Ja(le,C).length===0?ce:null;if(fe){let n={};for(let e of so){let t=C(e);t!==void 0&&(n[e]=t)}let a=[`deep-search`,t.name,`--filters`,JSON.stringify(n),`--data`,$t];f?a.push(`--motif`,ia(f)):d?a.push(`--material`,se(d)):(a.push(`--fen`,i),u!==`exact`&&a.push(`--match`,u));let o=r.prepare(`SELECT id, white, black, white_elo, black_elo, result, date, event, eco, opening, moves
         FROM games WHERE id = ?`);return e.header(`Content-Type`,`application/x-ndjson`),e.header(`X-Scan-Path`,`native`),Qi(e,n=>new Promise(r=>{let i;try{i=(0,sn.spawn)(fe,a,{cwd:X,stdio:[`ignore`,`pipe`,`pipe`]})}catch(e){console.error(`deep-search (${t.name}): could not start ${fe}: ${e.message}`),r();return}let s=()=>{i.kill()};e.req.raw.signal?.addEventListener(`abort`,s),n.onAbort(s);let c=0,l=0,u=``,p=Promise.resolve(),h=async e=>{let r;try{r=JSON.parse(e)}catch{console.error(`deep-search (${t.name}): unparseable frame from ${fe}: ${e.slice(0,120)}`);return}if(r.type===`hit`){if(c>=200)return;let e=o.get(r.id);if(!e){l+=1,console.error(`deep-search (${t.name}): native hit on a game that is not there: ${r.id}`);return}let i=f?va(e.moves,f):d?_a(e.moves,d):ha(e.moves,m);if(i!==r.ply&&(l+=1,console.error(`deep-search (${t.name}): native and the reference replay disagree on game ${r.id}: native ply ${r.ply}, replay ${i??`no hit`} — streaming the replay's answer`)),i===null)return;c+=1;let{moves:a,...s}=e;await n.writeln(JSON.stringify({type:`game`,ply:i,...s,...b(a)}))}else r.type===`progress`?await n.writeln(JSON.stringify({type:`progress`,scanned:r.scanned,total:r.total,matched:c})):r.type===`done`&&await n.writeln(JSON.stringify({type:`done`,scanned:r.scanned,total:r.total,matched:c,exhaustive:r.exhaustive}))};i.stdout.on(`data`,e=>{u+=e.toString();let t=u.split(`
...
`},$=`/vault`,Cs=`/demo/puzzles.sqlite`,ws=`/demo/refgames.sqlite`,Ts=`/demo/mygames.sqlite`,Es=`/demo/mygames-analysis.sqlite`,Ds=`.analysis.json`,Os=8,ks=`b5a3e1c07f2d49b8c`,As=`b7d4c1e93a06f52bd`,js=null,Ms=null,Ns=new Map;function Ps(){zt(),Bt();for(let[e,t]of Object.entries(Ss))e!==Ds&&F(`${$}/${e}`,t,Date.now()-Object.keys(Ss).indexOf(e)*36e5);let e=new G;rs(e,{studies:`${$}/studies`,notes:`${$}/notes`,games:`${$}/games`,puzzlesDb:Cs,puzzlesState:`${$}/puzzles`,refgamesDb:ws,myGamesDb:Ts,myGamesAnalysisDb:Es,repertoireState:`${$}/repertoire`,puzzleBooks:`${$}/puzzlebooks`,searchIndex:`${$}/.data/search-index.json`}),e.get(`/api/health`,e=>e.json({ok:!0,crossOriginIsolated:!1,version:`0.11.4`,build:`2026-09-20 15:49:21`,demo:!0}));let t={};return e.get(`/api/settings`,e=>e.json({profile:{},gate:!1,totp:!1,lichess:{configured:!1,last4:null},tablebase:{source:`lichess`,url:null,fallback:`https://tablebase.lichess.ovh/standard`,dir:null,local:!1,sameMachine:!1},training:t,vaultPath:`demo`,name:null,version:`demo`,demo:!0})),e.put(`/api/settings/training`,async e=>{let n=await e.req.json().catch(()=>null);return!n||typeof n!=`object`?e.json({error:`invalid training`},400):(t={...t,...m(n)},e.json({ok:!0}))}),e.get(`/api/explorer/:db`,async t=>{let n=t.req.query(`fen`)??``,r=await e.request(`/api/refgames/explore?fen=${encodeURIComponent(n)}`);if(!r.ok)return t.json({opening:null,moves:[],topGames:[]});let i=await r.json();return t.json({opening:null,moves:i.moves??[],topGames:i.topGames??[]})}),e.route(`/api`,_s(`${$}/puzzlebooks`,`${$}/books`)),e.route(`/api`,xs($,`${$}/.data`)),R($),e.route(`/api`,xn($,{run:Ce,available:he})),e.get(`/api/books/bookmarks`,e=>e.json({ids:[]})),e.post(`/api/books/bookmarks/toggle`,e=>e.json({ids:[]})),e.get(`/api/books`,e=>e.json({books:js?[{id:ks,title:`A sample book`,name:`sample.pdf`,bytes:js.byteLength,pages:Os,addedAt:new Date(Date.now()-864e5).toISOString(),lastPage:Ms,cover:!1,collection:null,puzzleBook:null}]:[],folders:[]})),e.get(`/api/books/:id/pdf`,e=>{if(e.req.param(`id`)!==ks||!js)return e.json({error:`unknown book`},404);let t=js.byteLength,n={"content-type":`application/pdf`,"accept-ranges":`bytes`,"cache-control":`private, no-cache`,etag:`"${t}-demo"`},r=/^bytes=(\d*)-(\d*)$/.exec(e.req.header(`range`)??``);if(r&&(r[1]||r[2])){let i=r[1]?Number(r[1]):Math.max(0,t-Number(r[2])),a=r[1]&&r[2]?Math.min(Number(r[2]),t-1):t-1;return i>a||i>=t?e.body(null,416,{"content-range":`bytes */${t}`}):e.body(js.slice(i,a+1),206,{...n,"content-range":`bytes ${i}-${a}/${t}`,"content-length":String(a-i+1)})}return e.body(js,200,{...n,"content-length":String(t)})}),e.put(`/api/books/:id/reading`,async e=>{if(e.req.param(`id`)!==ks)return e.json({error:`unknown book`},404);let t=await e.req.json().catch(()=>({})),n=Number(t.page);return!Number.isInteger(n)||n<1||n>Os?e.json({error:`bad page`},400):(Ms=n,e.json({ok:!0}))}),e.get(`/api/books/:id/diagrams`,e=>e.req.param(`id`)===ks?e.json({pages:Object.fromEntries(Ns)}):e.json({error:`unknown book`},404)),e.put(`/api/books/:id/diagrams/:page`,async e=>{if(e.req.param(`id`)!==ks)return e.json({error:`unknown book`},404);let t=await e.req.json().catch(()=>({}));return Ns.set(e.req.param(`page`),Array.isArray(t.diagrams)?t.diagrams:[]),e.json({ok:!0})}),e.all(`/api/*`,e=>e.json({error:`not available in the static demo`},404)),e}function Fs(){let e=Ss[Ds];if(e)try{let t=new Hr(Es),n=0;for(let r of JSON.parse(e)){let e=Ur(r);e?t.put(e):n+=1}n>0&&console.warn(`demo: ${n} analysis records refused`)}catch(e){console.warn(`demo: analysis seed unreadable —`,e)}}async function Is(){try{let e=await fetch(new URL(`demo/openings.json`,document.baseURI));e.ok&&F(tn,await e.text(),Date.now()),await Promise.all([`a`,`b`,`c`,`d`,`e`].map(async e=>{let t=await fetch(new URL(`demo/eco/${e}.tsv`,document.baseURI));t.ok&&F(`${X}/scripts/vendor/chess-openings/${e}.tsv`,await t.text(),Date.now())}))}catch{}try{let e=await fetch(new URL(`demo/books/sample.pdf`,document.baseURI),{cache:`no-cache`});e.ok?(js=new Uint8Array(await e.arrayBuffer()),F(`${$}/books/${ks}/book.pdf`,`served from memory — see installDemoBackend`,Date.now())):console.warn(`demo: no sample book (${e.status})`)}catch(e){console.warn(`demo: sample book unavailable —`,e)}try{for(let e of[`page003.jpg`,`page005.jpg`,`page007.jpg`]){let t=await fetch(new URL(`demo/book-pages/${e}`,document.baseURI));if(!t.ok)continue;let n=new Uint8Array(await t.arrayBuffer());_e(`${$}/puzzlebooks/${As}/diagrams/${e}`,n,Date.now()),pe(As,e,URL.createObjectURL(new Blob([n],{type:`image/jpeg`})))}}catch(e){console.warn(`demo: book pages unavailable —`,e)}try{await Tn({[Cs]:`demo/puzzles.sqlite`,[ws]:`demo/refgames.sqlite`})}catch(e){console.warn(`demo: puzzles and reference games unavailable —`,e)}Fs();let e=Ps(),t=window.fetch.bind(window);window.fetch=async(n,r)=>{let i=new Request(n,r),a=new URL(i.url,window.location.href);return a.origin!==window.location.origin||!a.pathname.startsWith(`/api/`)?t(n,r):e.fetch(new Request(a.toString(),i))}}export{Is as installDemoBackend};